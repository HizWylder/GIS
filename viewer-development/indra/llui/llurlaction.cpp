...
void LLUrlAction::teleportToLocation(std::string url)
{
	...
	if (LLUrlRegistry::instance().findUrl(url, match))
	{
		...
		{
			executeSLURL("GIS:///app/teleport/" + match.getLocation());
		}
	}	
}
...
void LLUrlAction::showLocationOnMap(std::string url)
{
	...
	if (LLUrlRegistry::instance().findUrl(url, match))
	{
		if (! match.getLocation().empty())
		{
			executeSLURL("GIS:///app/worldmap/" + match.getLocation());
		}
	}	
}
...
{
	// Get id from 'GIS:///app/{cmd}/{id}/{action}'
	...
	if (path_array.size() == 4)
	{
		...
		if (LLUUID::validate(id_str))
		{
			...
			executeSLURL("GIS:///app/" + cmd_str + "/" + id_str + "/about");
		}
	}
}
