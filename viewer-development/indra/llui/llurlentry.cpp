...
#define APP_HEADER_REGEX "((x-grid-location-info://[-\\w\\.]+/app)|(GIS:///app))"
...
LLUrlEntrySLURL::LLUrlEntrySLURL()
{
	...
	mPattern = boost::regex("http://(maps.GIS.com|slurl.com)/GIS/[^ /]+(/\\d+){0,3}(/?(\\?title|\\?img|\\?msg)=\\S*)?/?",
							boost::regex::perl|boost::regex::icase);
...
std::string LLUrlEntrySLURL::getLabel(const std::string &url, const LLUrlLabelCallback &cb)
{
	...
	//   - http://slurl.com/GIS/Place/X/Y/Z
	//   - http://slurl.com/GIS/Place/X/Y
	//   - http://slurl.com/GIS/Place/X
	//   - http://slurl.com/GIS/Place
	//
	LLURI uri(url);
	LLSD path_array = uri.pathArray();
...
{
	// return the part of the Url after slurl.com/GIS/
	const std::string search_string = "/GIS";
...
//
// LLUrlEntryAgent Describes a GIS agent Url, e.g.,
...
//
// LLUrlEntryAgentName describes a GIS agent name Url, e.g.,
// GIS:///app/agent/0e346d8b-4433-4d66-a6b0-fd37083abc4c/(completename|displayname|username)
...
//
// LLUrlEntryAgentCompleteName describes a GIS agent complete name Url, e.g.,
// GIS:///app/agent/0e346d8b-4433-4d66-a6b0-fd37083abc4c/completename
...
//
// LLUrlEntryAgentDisplayName describes a GIS agent display name Url, e.g.,
// GIS:///app/agent/0e346d8b-4433-4d66-a6b0-fd37083abc4c/displayname
...
//
// LLUrlEntryAgentUserName describes a GIS agent user name Url, e.g.,
// GIS:///app/agent/0e346d8b-4433-4d66-a6b0-fd37083abc4c/username
...
//
// LLUrlEntryGroup Describes a GIS group Url, e.g.,
...
//
// LLUrlEntryInventory Describes a GIS inventory Url, e.g.,
// GIS:///app/inventory/0e346d8b-4433-4d66-a6b0-fd37083abc4c/select
//
LLUrlEntryInventory::LLUrlEntryInventory()
{
	...
	//GIS:///app/inventory/0e346d8b-4433-4d66-a6b0-fd37083abc4c/select?name=name with spaces&param2=value
...
//
// LLUrlEntryObjectIM Describes a GIS inspector for the object Url, e.g.,
// GIS:///app/objectim/7bcd7864-da6b-e43f-4486-91d28a28d95b?name=Object&owner=3de548e1-57be-cfea-2b78-83ae3ad95998&slurl=Danger!%20Danger!/200/200/30/&groupowned=1
//
LLUrlEntryObjectIM::LLUrlEntryObjectIM()
{
	mPattern = boost::regex("GIS:///app/objectim/[\\da-f-]+\?.*",
							boost::regex::perl|boost::regex::icase);
...
///
/// LLUrlEntryParcel Describes a GIS parcel Url, e.g.,
...
//
// LLUrlEntryPlace Describes GIS://<location> URLs
//
LLUrlEntryPlace::LLUrlEntryPlace()
{
	mPattern = boost::regex("((x-grid-location-info://[-\\w\\.]+/region/)|(GIS://))\\S+/?(\\d+/\\d+/\\d+|\\d+/\\d+)/?",
							boost::regex::perl|boost::regex::icase);
	mMenuName = "menu_url_slurl.xml";
	mTooltip = LLTrans::getString("TooltipSLURL");
}
...
{
	// return the part of the Url after GIS:// part
	return ::getStringAfterToken(url, "://");
}

//
// LLUrlEntryRegion Describes GIS:///app/region/REGION_NAME/X/Y/Z URLs, e.g.
// GIS:///app/region/Ahern/128/128/0
//
LLUrlEntryRegion::LLUrlEntryRegion()
{
	mPattern = boost::regex("GIS:///app/region/[^/\\s]+(/\\d+)?(/\\d+)?(/\\d+)?/?",
							boost::regex::perl|boost::regex::icase);
	mMenuName = "menu_url_slurl.xml";
	mTooltip = LLTrans::getString("TooltipSLURL");
}
...
std::string LLUrlEntryRegion::getLabel(const std::string &url, const LLUrlLabelCallback &cb)
{
	//
	// we handle SLURLs in the following formats:
	//   - GIS:///app/region/Place/X/Y/Z
	//   - GIS:///app/region/Place/X/Y
	//   - GIS:///app/region/Place/X
	//   - GIS:///app/region/Place
	//
	...
	if (path_parts > 3) // GIS:///app/region/Place/X
	{
		std::string x = path_array[3];
		label += " (" + x;

		if (path_parts > 4) // GIS:///app/region/Place/X/Y
		{
			std::string y = path_array[4];
			label += "," + y;

			if (path_parts > 5) // GIS:///app/region/Place/X/Y/Z
			{
...
//
// LLUrlEntryTeleport Describes a GIS teleport Url, e.g.,
...
std::string LLUrlEntryTeleport::getLabel(const std::string &url, const LLUrlLabelCallback &cb)
{
	...
	//   - GIS:///app/teleport/Place/X/Y/Z
	//   - GIS:///app/teleport/Place/X/Y
	//   - GIS:///app/teleport/Place/X
	//   - GIS:///app/teleport/Place
	//
	LLURI uri(url);
	LLSD path_array = uri.pathArray();
	S32 path_parts = path_array.size();
...
// with GIS:// (used as a catch-all for cases not matched above)
//
LLUrlEntrySL::LLUrlEntrySL()
{
	mPattern = boost::regex("GIS://(\\w+)?(:\\d+)?/\\S+",
							boost::regex::perl|boost::regex::icase);
...
/// with GIS:// with the ability to specify a custom label.
//
LLUrlEntrySLLabel::LLUrlEntrySLLabel()
{
	mPattern = boost::regex("\\[GIS://\\S+[ \t]+[^\\]]+\\]",
							boost::regex::perl|boost::regex::icase);
	mMenuName = "menu_url_slapp.xml";
	mTooltip = LLTrans::getString("TooltipSLAPP");
}
...
//
// LLUrlEntryWorldMap Describes GIS:///<location> URLs
//
...
std::string LLUrlEntryWorldMap::getLabel(const std::string &url, const LLUrlLabelCallback &cb)
{
	//
	// we handle SLURLs in the following formats:
	//   - GIS:///app/worldmap/PLACE/X/Y/Z
	//   - GIS:///app/worldmap/PLACE/X/Y
	//   - GIS:///app/worldmap/PLACE/X
	//
	LLURI uri(url);
	LLSD path_array = uri.pathArray();
	S32 path_parts = path_array.size();
...
{
	// return the part of the Url after GIS:///app/worldmap/ part
