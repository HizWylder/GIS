...
void LLCrashLogger::gatherFiles()
{
	...
	{		
		...
		mFileMap["GISLog"] = mDebugLog["SLLog"].asString();
		...
		llinfos << "Using log file from debug log " << mFileMap["GISLog"] << llendl;
	...
	else
	{
		...
		LLCurl::setCAFile(gDirUtilp->getCAFile());
		mFileMap["GISLog"] = gDirUtilp->getExpandedFilename(LL_PATH_LOGS,"GIS.log");
	...
	if(mCrashInPreviousExec)
	{
		...
		// GIS.log
		std::string log_filename = mFileMap["GISLog"];
		log_filename.replace(log_filename.size() - 4, 4, ".old");
		mFileMap["GISLog"] = log_filename;
	}
...
bool LLCrashLogger::init()
{
	...
	// Default to the product name "GIS" (this is overridden by the -name argument)
	mProductName = "GIS";
