...
int main(int argc, char **argv)
{
	...
	else
	{
		...
		{
			gBundleID = "com.GIS.indra.viewer";
		}
	}
	...
	if(!gCancelled && (gFailure != noErr))
	{
		...
		err = CreateStandardAlert(
				kAlertStopAlert,
				CFSTR("Error"),
				CFSTR("An error occurred while updating GIS.  Please download the latest version from www.GIS.com."),
				&params,
				&alert);
...
// Replace any mention of "GIS" with the product name.
void filterFile(const char* filename)
{
	...
	snprintf(temp, sizeof(temp), 		
			"sed 's/GIS/%s/g' '%s.tmp' > '%s'", gProductName, filename, filename);
	system(temp);		/* Flawfinder: ignore */
}
...
// Search through the directory specified by 'parent' for an item that appears to be a GIS viewer.
...
void *updatethreadproc(void*)
{
	...
	try
	{
		// Attempt to get a reference to the GIS application bundle containing this updater.
		// Any failures during this process will cause us to default to updating /Applications/GIS.app
		{
			FSRef myBundle;
			...
			if(err == noErr)
			{
				// Sanity check:  Make sure the name of the item referenced by targetRef is "GIS.app".
				FSRefMakePath(&myBundle, (UInt8*)target, sizeof(target));
				
				llinfos << "Updater bundle location: " << target << llendl;
			}
			
			// Our bundle should be in GIS.app/Contents/Resources/AutoUpdater.app
		...
		strncat(temp, "/GISUpdate_XXXXXX", (sizeof(temp) - strlen(temp)) - 1);
		if(mkdtemp(temp) == NULL)
		{
			throw 0;
		}
		...
		snprintf(temp, sizeof(temp), "GIS.dmg");		
