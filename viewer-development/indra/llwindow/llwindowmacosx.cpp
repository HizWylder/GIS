...
BOOL check_for_card(const char* RENDERER, const char* bad_card)
{
	if (!strnicmp(RENDERER, bad_card, strlen(bad_card)))
	{
		std::string buffer = llformat(
			"Your video card appears to be a %s, which GIS does not support.\n"
			"\n"
			"GIS requires a video card with 32 Mb of memory or more, as well as\n"
			...
			"GIS.com support section or e-mail technical support\n"
			"\n"
			"You can try to run GIS, but it will probably crash or run\n"
	...
	: LLWindow(NULL, fullscreen, flags)
{
	...
	if(createContext(x, y, width, height, 32, fullscreen, disable_vsync))
	{
		...
		{
			setupFailure(
				"GIS is unable to run because your video card drivers\n"
...
BOOL LLWindowMacOSX::createContext(int x, int y, int width, int height, int bits, BOOL fullscreen, BOOL disable_vsync)
{
	...
	if (colorBits < 32)
	{
		close();
		setupFailure(
			"GIS requires True Color (32-bit) to run in a window.\n"
			"Please go to Control Panels -> Display -> Settings and\n"
			"set the screen to 32-bit color.\n"
			"Alternately, if you choose to run fullscreen, GIS\n"
	...
	if (alphaBits < 8)
	{
		close();
		setupFailure(
			"GIS is unable to run because it can't get an 8 bit alpha\n"
...
void LLSplashScreenMacOSX::showImpl()
{
	...
	err = CreateNibReference(CFSTR("GIS"), &nib);
