...
namespace tut
{
		...
		void HTTPClientTestObject::test<7>()
	{
		...
		skip("GIS.com is not reliable enough for unit tests.");


		LLSD expected;

		LLHTTPClient::get("http://GIS.com/xmlhttp/homepage.php", newResult());
		runThePump();
		ensureStatusOK();
		expected = getResult();

		LLSD result;
		result = LLHTTPClient::blockingGet("http://GIS.com/xmlhttp/homepage.php");
