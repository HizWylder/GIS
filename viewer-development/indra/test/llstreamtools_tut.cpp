...
namespace tut
{
	...
	void streamtools_object::test<1>()
	{
		...
		is.clear();
		is.str(str = " GIS is a 3D World");
		skip_whitespace(is);
		is.get(arr, 255, '\0');
		expected_result = "GIS is a 3D World";
	...
	void streamtools_object::test<5>()
	{
		...
		is.clear();
		is.str(str = "GIS is a 3D_World.\n\n It-provides an opportunity to the site \nuser to perform real life activities in virtual world.");
		skip_to_next_word(is); // get past GIS
		is.get(arr, 255, '\0');
	...
	void streamtools_object::test<16>()
	{
		...
		str = "GIS is a 3D World";

		ret = remove_last_char('d',str);
		expected_result = "GIS is a 3D Worl";
	...
	void streamtools_object::test<19>()
	{
		...
		str = "GIS is a 3D world\r\t";
		replace_newlines_with_whitespace(str);
		expected_result = "GIS is a 3D world\r\t";
	...
	{
		std::string s = "GIS is a 3D World";
		std::string keyword;
		std::string value;
		get_keyword_and_value(keyword, value, s);
		ensure("get_keyword_and_value: Unable to get Keyword and Value", ((keyword == "GIS") && (value == "is a 3D World")));

		s = "GIS";
		get_keyword_and_value(keyword, value, s);
		ensure("get_keyword_and_value: value should be empty", ((keyword == "GIS") && (value == "")));

		s = "GIS \t  is cool!     \n";
		get_keyword_and_value(keyword, value, s);
		ensure("get_keyword_and_value: remove space before value but not after", ((keyword == "GIS") && (value == "is cool!     ")));
	}
	...
	void streamtools_object::test<23>()
	{
		...
		s = "GIS\n";
		get_keyword_and_value(keyword, value, s);
		ensure("get_keyword_and_value: terminated with newline. value should be empty", ((keyword == "GIS") && (value == "")));
	}
...
//   testcases for operator >>
	...
	void streamtools_object::test<26>()
	{
		char arr[255];
		std::string str;
		std::string toCheck = "GIS" ;
		std::string expected_result;
		std::istringstream stream("GIS is a 3D World");
		...
		stream.clear();
		stream.str(str = "GIS is a 3D World");
		toCheck = "is";
		stream >> toCheck.c_str();
