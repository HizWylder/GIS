...
namespace tut
{
	...
	void string_index_object_t::test<9>()
	{
		...
		ensure_equals("toLower failed", str_val, "GIS source");
	}
	...
	void string_index_object_t::test<10>()
	{
		...
		ensure("1. isHead failed", LLStringUtil::isHead(str_val, "GIS Source") == TRUE);
		ensure("2. isHead failed", LLStringUtil::isHead(str_val, " GIS Source") == FALSE);
		std::string str_val2("");
