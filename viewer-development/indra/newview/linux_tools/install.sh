...
# Install the GIS Viewer. This script can install the viewer both
...
function homedir_install()
{
    ...
    warn "to install the GIS Viewer in your home directory. If you"
    warn "would like to install the GIS Viewer system-wide, please run"
    ...
    install_to_prefix "$HOME/.GIS-install"
    $HOME/.GIS-install/etc/refresh_desktop_app_entry.sh
}

function root_install()
{
    local default_prefix="/opt/GIS-install"
