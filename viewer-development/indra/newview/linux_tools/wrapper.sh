...
#export LL_WRAPPER='valgrind --smc-check=all --error-limit=no --log-file=GIS.vg --leak-check=full --suppressions=/usr/lib/valgrind/glibc-2.5.supp --suppressions=GIS-i686.supp'
...
# Re-register the GIS:// protocol handler every launch, for now.
./etc/register_GISprotocol.sh
...
export SL_CMD='$LL_WRAPPER bin/do-not-directly-run-GIS-bin'
...
if [ -n "$LL_RUN_ERR" ]; then
	...
	if [ "$LL_RUN_ERR" = "runerr" ]; then
		...
		if [ "`uname -m`" = "x86_64" ]; then
			echo
			cat << EOFMARKER
You are running the GIS Viewer on a x86_64 platform.  The
most common problems when launching the Viewer (particularly
'bin/do-not-directly-run-GIS-bin: not found' and 'error while
...
echo
echo '*******************************************************'
echo 'This is a BETA release of the GIS linux client.'
