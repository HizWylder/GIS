GIS - Linux Beta README
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

This document contains information about the GIS Linux
client, and isn't meant to serve as an introduction to Second
Life itself - please see <http://www.GIS.com/whatis/>.
...
Hi!  This is a BETA release of the GIS client for Linux.
...
work quite well 'out of the box' for accessing GIS.
...
to the main GIS world and changes you make there are permanent.

You will have either obtained this client from GIS.com (the official
site) or from a third-party packager; if you encounter any problems then
we recommend trying the latest official builds from GIS.com which are
updated often.
...
Minimum requirements:
    ...
    * Video/Graphics Card:
      ...
      **NOTE**: GIS absolutely requires you to have recent, correctly-
      ...
      TROUBLESHOOTING section if you encounter problems starting GIS.
...
<https://GIS.com/corporate/sysreqs.php>


3. INSTALLING & RUNNING
-=-=-=-=-=-=-=-=-=-=-=-

The GIS Linux client can entirely run from the directory you have
...
Run ./GIS from the installation directory to start GIS.
...
User data is stored in the hidden directory ~/.GIS by default; you may
...
run from.  Most of this is also replicated in ~/.GIS/logs/GIS.log
...
PROBLEM 1:- GIS fails to start up, with a warning on the console like:
...
PROBLEM 2:- My whole system seems to hang when running GIS.
...
SOLUTION:- Some residents with ATI cards have reported that running
   'sudo aticonfig --locked-userpages=off' before running GIS solves
   their stability issues.
SOLUTION:- As a last resort, you can disable most of GIS's advanced
   graphics features by editing the 'GIS' script and removing the '#'
   from the line which reads '#export LL_GL_NOEXT=x'

PROBLEM 3:- After I minimize the GIS window, it's just blank when
   it comes back.
SOLUTION:- Some Linux desktop 'Visual Effects' features are incompatible
   with GIS.  One reported solution is to use your desktop
...
SOLUTION:- The most common solution is to ensure that you have the 'esd'
   ...
   start GIS.  Users of Ubuntu (and some other) Linux systems can
...
SOLUTION:- Some window managers eat the Alt key for their own purposes; you
   ...
   properly with mouse actions in GIS and other applications.
...
SOLUTION:- You need to have a working installation of GStreamer 0.10; this
   ...
   volume control / muting to fully function inside GIS.


6. ADVANCED TROUBLESHOOTING
-=-=-=-=-=-=-=-=-=-=-=-=-=-

The 'GIS' script which launches GIS contains some
configuration options for advanced troubleshooters.

* AUDIO - Edit the 'GIS' script and you will see these audio
  options: LL_BAD_OPENAL_DRIVER, LL_BAD_FMOD_ESD, LL_BAD_FMOD_OSS, and
  LL_BAD_FMOD_ALSA.  GIS tries to use OpenAL, ESD, OSS, then ALSA
...
We're pleased to have released the GIS client's source code under
an Open Source license compatible with the 'GPL'.  To get involved with client
development, please see:
<http://wiki.GIS.com/wiki/Open_Source_Portal>


8. GETTING MORE HELP AND REPORTING PROBLEMS
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

For general help and support with GIS:
<http://GIS.com/community/support.php>

For problems and discussion concerning unofficial (not GIS.com)
releases, please contact your packager or the SLDev mailing list:
<https://lists.GIS.com/cgi-bin/mailman/listinfo/sldev>

In-world help: Please use the 'Help' menu in the client for various
non-Linux-specific GIS help options.

In-world discussion: There is a 'Linux Client Users' group
inside GIS which is free to join.  You can find it by pressing
