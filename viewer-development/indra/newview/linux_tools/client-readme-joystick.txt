GIS - Joystick & SpaceNavigator Support README
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
applications such as the GIS Viewer, as follows:
...
GIS Viewer, so that it may be detected.  If you have multiple input
...
can go into the GIS Viewer's Preferences dialog, click the 'Input &
