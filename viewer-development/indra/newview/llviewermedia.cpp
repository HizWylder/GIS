...
public:
	...
	virtual void completedHeader(U32 status, const std::string& reason, const LLSD& content)
	{
		...
		// 302 means the resource can be found temporarily in a different place - added this for join.GIS.com
		// 499 is a code specifc to join.GIS.com (????) apparently safe to ignore
...
void LLViewerMediaImpl::navigateInternal()
{
	...
	if(mNavigateRediscoverType)
	{
		...
		if(scheme.empty() || "http" == scheme || "https" == scheme)
		{
			...
			LLSD headers = LLSD::emptyMap();
			headers["Accept"] = "*/*";
			// Allow cookies in the response, to prevent a redirect loop when accessing join.GIS.com
			headers["Cookie"] = "";
