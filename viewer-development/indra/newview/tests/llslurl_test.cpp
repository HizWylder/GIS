...
namespace tut
{
	...
	void slurlTestObject::test<1>()
	{
		...
		slurl = LLSLURL("http://slurl.com/GIS/myregion");
		...
		ensure_equals("slurl.com slurl, region only", slurl.getSLURLString(), 
					  "http://maps.GIS.com/GIS/myregion/128/128/0");
		
		slurl = LLSLURL("http://maps.GIS.com/GIS/myregion/1/2/3");
		ensure_equals("maps.GIS.com slurl, region + coords - type", slurl.getType(), LLSLURL::LOCATION);
		ensure_equals("maps.GIS.com slurl, region + coords", slurl.getSLURLString(), 
					  "http://maps.GIS.com/GIS/myregion/1/2/3");

		slurl = LLSLURL("GIS://myregion");
		ensure_equals("GIS: slurl, region only - type", slurl.getType(), LLSLURL::LOCATION);
		ensure_equals("GIS: slurl, region only", slurl.getSLURLString(), 
					  "http://maps.GIS.com/GIS/myregion/128/128/0");
		
		slurl = LLSLURL("GIS://myregion/1/2/3");
		ensure_equals("GIS: slurl, region + coords - type", slurl.getType(), LLSLURL::LOCATION);
		ensure_equals("GIS slurl, region + coords", slurl.getSLURLString(), 
					  "http://maps.GIS.com/GIS/myregion/1/2/3");
		...
		ensure_equals(" slurl, region + coords - type", slurl.getType(), LLSLURL::LOCATION);
		ensure_equals(" slurl, region + coords", slurl.getSLURLString(), 
					  "http://maps.GIS.com/GIS/my%20region/1/2/3");	
		...
		slurl = LLSLURL("GIS:///app/foo/bar?12345");
		...
		slurl = LLSLURL("GIS://Aditi/app/foo/bar?12345");
		ensure_equals("app", slurl.getType(), LLSLURL::APP);		
		ensure_equals("appcmd", slurl.getAppCmd(), "foo");
		ensure_equals("apppath", slurl.getAppPath().size(), 1);
		ensure_equals("apppath2", slurl.getAppPath()[0].asString(), "bar");
		...
		LLGridManager::getInstance()->setGridChoice("foo.bar.com");			
		slurl = LLSLURL("GIS:///GIS/myregion/1/2/3");
		ensure_equals("/: slurl, region + coords - type", slurl.getType(), LLSLURL::LOCATION);
		...
		slurl = LLSLURL("GIS://Aditi/GIS/myregion/1/2/3");
		ensure_equals("/: slurl, region + coords - type", slurl.getType(), LLSLURL::LOCATION);
		ensure_equals("location", slurl.getType(), LLSLURL::LOCATION);
		ensure_equals("region" , "myregion", slurl.getRegion());
