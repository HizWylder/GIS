...
void LLAppViewerMacOSX::handleCrashReporting(bool reportFreeze)
{
	...
	//command_str = "open GIS.app/Contents/Resources/mac-crash-logger.app";
...
OSErr AEGURLHandler(const AppleEvent *messagein, AppleEvent *reply, long refIn)
{
	...
	if(result == noErr)
	{
		...
		// Safari 3.2 silently mangles GIS:///app/ URLs into
		// GIS:/app/ (only one leading slash).
		// Fix them up to meet the URL specification. JC
		const std::string prefix = "GIS:/app/";
		std::string test_prefix = url.substr(0, prefix.length());
		LLStringUtil::toLower(test_prefix);
		if (test_prefix == prefix)
		{
			url.replace(0, prefix.length(), "GIS:///app/");
		}
...
void init_apple_menu(const char* product)
{
	...
	// Install a handler for 'gurl' AppleEvents.  This is how GIS:// URLs get passed to the viewer.
