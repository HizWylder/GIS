...
;; !define INSTNAME   "GIS%(grid_caps)s"
;; !define SHORTCUT   "GIS (%(grid_caps)s)"
;; !define URLNAME   "GIS%(grid)s"
...
Function CloseGIS
  ...
  StrCmp $SKIP_DIALOGS "true" CLOSE
    MessageBox MB_OKCANCEL $(CloseGISInstMB) IDOK CLOSE IDCANCEL CANCEL_INSTALL

  CANCEL_INSTALL:
    Quit

  CLOSE:
    DetailPrint $(CloseGISInstDP)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Test our connection to GIS.com
...
Function CheckNetworkConnection
    ...
    NSISdl::download_quiet \
        ...
        "http://install.GIS.com/check/?stubtag=$2&version=${VERSION_LONG}" \
        $0
...
; GISViewer2 and GISViewer installations existing side
...
Function CheckWillUninstallV2
  ...
  StrCmp $INSTDIR "$PROGRAMFILES\GISViewer2" CHECKV2_DONE ; don't uninstall our own install dir.
  IfFileExists "$PROGRAMFILES\GISViewer2\uninst.exe" CHECKV2_FOUND CHECKV2_DONE
...
Push $2
    ...
    RMDir /r "$TEMP\GISSettingsBackup"
    CreateDirectory "$TEMP\GISSettingsBackup"
  ...
  LOOP:
    ...
    CreateDirectory "$TEMP\GISSettingsBackup\$0"
    CopyFiles /SILENT "$2\Application Data\GIS\*" "$TEMP\GISSettingsBackup\$0"
...
; Copy files in Documents and Settings\All Users\GIS
Push $0
    ReadRegStr $0 HKEY_LOCAL_MACHINE "SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders" "Common AppData"
    StrCmp $0 "" +2
    CreateDirectory "$TEMP\GISSettingsBackup\AllUsers\"
    CopyFiles /SILENT "$2\Application Data\GIS\*" "$TEMP\GISSettingsBackup\AllUsers\"
Pop $0
...
Push $2
  ...
  LOOP:
    ...
    CreateDirectory "$2\Application Data\GIS\"
    CopyFiles /SILENT "$TEMP\GISSettingsBackup\$0\*" "$2\Application Data\GIS\" 
...
Push $0
    ...
    StrCmp $0 "" +2
    CreateDirectory "$2\Application Data\GIS\"
    CopyFiles /SILENT "$TEMP\GISSettingsBackup\AllUsers\*" "$2\Application Data\GIS\" 
Pop $0
...
Push $2
  ...
  LOOP:
    ...
    RMDir /r "$TEMP\GISSettingsBackup\$0\*"
...
Push $0
    ReadRegStr $0 HKEY_LOCAL_MACHINE "SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders" "Common AppData"
    StrCmp $0 "" +2
    RMDir /r "$TEMP\GISSettingsBackup\AllUsers\*"
Pop $0
...
;    ExpandEnvStrings $2 $2
;
;    RMDir /r "$2\Application Data\GIS\"
;
;  CONTINUE:
;    IntOp $0 $0 + 1
;    Goto LOOP
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Delete files in Documents and Settings\<user>\GIS
; Delete files in Documents and Settings\All Users\GIS
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;    RMDir /r "$2\Application Data\GIS\logs"
    RMDir /r "$2\Application Data\GIS\browser_profile"
    RMDir /r "$2\Application Data\GIS\user_settings"
    Delete  "$2\Application Data\GIS\*.xml"
    Delete  "$2\Application Data\GIS\*.bmp"
    Delete  "$2\Application Data\GIS\search_history.txt"
    Delete  "$2\Application Data\GIS\plugin_cookies.txt"
    Delete  "$2\Application Data\GIS\typed_locations.txt"
...
; Delete files in C:\Windows\Application Data\GIS
; If the user is running on a pre-NT system, Application Data lives here instead of
; in Documents and Settings.
RMDir /r "$WINDIR\Application Data\GIS"
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
Function un.CloseGIS
  Push $0
  FindWindow $0 "GIS" ""
  IntCmp $0 0 DONE
  MessageBox MB_OKCANCEL $(CloseGISUnInstMB) IDOK CLOSE IDCANCEL CANCEL_UNINSTALL

  CANCEL_UNINSTALL:
    Quit

  CLOSE:
    DetailPrint $(CloseGISUnInstDP)
    SendMessage $0 16 0 0
...
DetailPrint "Removing GIS password"

SetShellVarContext current
Delete "$APPDATA\GIS\user_settings\password.dat"
...
Call CloseGIS			; Make sure we're not running
Call CheckNetworkConnection		; ping GIS.com
...
WriteRegStr HKEY_CLASSES_ROOT "${URLNAME}" "(default)" "URL:GIS"
...
WriteRegStr HKEY_CLASSES_ROOT "x-grid-location-info"(default)" "URL:GIS"
...
; Uninstall existing "GIS Viewer 2" install if needed.
StrCmp $DO_UNINSTALL_V2 "" REMOVE_SLV2_DONE
  ExecWait '"$PROGRAMFILES\GISViewer2\uninst.exe" /S _?=$PROGRAMFILES\GISViewer2'
  Delete "$PROGRAMFILES\GISViewer2\uninst.exe" ; with _? option above, uninst.exe will be left behind.
  RMDir "$PROGRAMFILES\GISViewer2" ; will remove only if empty.
