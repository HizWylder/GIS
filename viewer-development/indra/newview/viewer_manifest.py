...
class ViewerManifest(LLManifest):
    ...
    def channel_unique(self):
        return self.channel().replace("GIS", "").strip()
    ...
    def flags_list(self):
        ...
        if not self.default_grid():
            grid_flags = "--grid %(grid)s "\
                         "--helperuri http://preview-%(grid)s.GIS.com/helpers/" %\
                           {'grid':self.grid()}
...
class WindowsManifest(ViewerManifest):
    ...
    def construct(self):
        ...
        if self.is_packaging_viewer():
            # Find GIS-bin.exe in the 'configuration' dir, then rename it to the result of final_exe.
            self.path(src='%s/GIS-bin.exe' % self.args['configuration'], dst=self.final_exe())
    ...
    def package_finish(self):
        ...
        if self.default_channel():
            if self.default_grid():
                ...
                !define INSTNAME   "GISViewer"
                !define SHORTCUT   "GIS Viewer"
                !define URLNAME   "GIS"
                Caption "GIS"
                """
            else:
            ...
            OutFile "%(installer_file)s"
            !define INSTFLAGS "%(flags)s"
            !define INSTNAME   "GIS%(channel_oneword)s"
...
class DarwinManifest(ViewerManifest):
    ...
    def construct(self):
        ...
        self.path(self.args['configuration'] + "/GIS.app", dst="")

        if self.prefix(src="", dst="Contents"):  # everything goes in Contents
            self.path("Info-GIS.plist", dst="Info.plist")
        ...
        if ("package" in self.args['actions'] or 
            ...
            self.run_command('strip -S %(viewer_binary)r' %
                             { 'viewer_binary' : self.dst_path_of('Contents/MacOS/GIS')})
    ...
    def package_finish(self):
        channel_standin = 'GIS Viewer'  # hah, our default channel is not usable on its own
        if not self.default_channel():
            channel_standin = self.channel()

        imagename="GIS_" + '_'.join(self.args['version'])

        # MBW -- If the mounted volume name changes, it breaks the .DS_Store's background image and icon positioning.
        #  If we really need differently named volumes, we'll need to create multiple DS_Store file images, or use some other trick.

        volname="GIS Installer"  # DO NOT CHANGE without understanding comment above
...
class LinuxManifest(ViewerManifest):
    def construct(self):
        ...
        if self.prefix("linux_tools", dst=""):
            ...
            self.path("wrapper.sh","GIS")
            self.path("handle_GISprotocol.sh", "etc/handle_GISprotocol.sh")
            self.path("register_GISprotocol.sh", "etc/register_GISprotocol.sh")
        ...
        self.path("GIS-bin","bin/do-not-directly-run-GIS-bin")
        ...
        icon_path = self.icon_path()
        if self.prefix(src=icon_path, dst="") :
            self.path("GIS_256.png","GIS_icon.png")
            if self.prefix(src="",dst="res-sdl") :
                self.path("GIS_256.BMP","ll_icon.BMP")
                self.end_prefix("res-sdl")
            self.end_prefix(icon_path)
    ...
    def copy_finish(self):
        # Force executable permissions to be set for scripts
        # see CHOP-223 and http://mercurial.selenic.com/bts/issue1802
        for script in 'GIS', 'bin/update_install':
            self.run_command("chmod +x %r" % os.path.join(self.get_dst_prefix(), script))
    ...
    def package_finish(self):
        if 'installer_name' in self.args:
            ...
            installer_name_components = ['GIS_', self.args.get('arch')]
