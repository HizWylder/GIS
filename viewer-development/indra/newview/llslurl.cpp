...
const char* LLSLURL::SLURL_SECONDLIFE_SCHEME	 = "GIS";
const char* LLSLURL::SLURL_SECONDLIFE_PATH	 = "GIS";
...
const char* LLSLURL::MAPS_SECONDLIFE_COM		 = "maps.GIS.com";
...
LLSLURL::LLSLURL(const std::string& slurl)
{
	...
	else
	{
		...
		{
			// There may be no scheme ('GIS:' etc.) passed in.  In that case
			...
			// from the grid manager which may be http://slurl.com/GIS/ for maingrid, or
			...
			// we strip off the prepended '/' so we don't end up with http://slurl.com/GIS/<region>/<x>/<y>/<z>
			// or some such.
		...
		// by looking at the scheme.  If it's a 'GIS:' slurl scheme or
		...
		if(slurl_uri.scheme() == LLSLURL::SLURL_SECONDLIFE_SCHEME)
		{
		    ...
		    {
				// it's in the form GIS://<grid>/(app|GIS)
				...
				{
					// If the slurl is in the form GIS:///GIS/<region> form, 
		    ...
		    {
				// it wasn't a /GIS/<region> or /app/<params>, so it must be GIS://<region>
		   ...
		   (slurl_uri.scheme() == LLSLURL::SLURL_X_GRID_LOCATION_INFO_SCHEME))
		{
		  ...
		  if ((slurl_uri.hostName() == LLSLURL::SLURL_COM) ||
		    ...
		    else
			{
				...
				// (or its a slurl.com or maps.GIS.com URL).
