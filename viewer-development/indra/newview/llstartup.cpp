...
bool idle_startup()
{
	...
	if ( STATE_FIRST == LLStartUp::getStartupState() )
	{
		...
		found_template = LLFile::fopen(message_template_path, "r");		/* Flawfinder: ignore */
			...
			// indra/build-*/newview/<config>/GIS/Contents/Resources/app_settings
			if (!found_template)
			{
...
void login_show()
{
	...
	LLPanelLogin::show(	gViewerWindow->getWindowRectScaled(),
						bUseDebugLogin || gSavedSettings.getBOOL("GISEnterprise"),
						login_callback, NULL );
...
bool process_login_success_response()
{
	...
	{
		gSavedSettings.setString("MapServerURL", "http://test.map.GIS.com.s3.amazonaws.com/");
	}
