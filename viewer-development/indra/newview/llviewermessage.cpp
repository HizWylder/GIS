	...
	else //has not expired
	{
			...
			&& throttle_count >= OFFER_THROTTLE_MAX_COUNT)
		{
			if (!throttle_logged)
			{
				...
				arg["APP_NAME"] = LLAppViewer::instance()->getGISTitle();
				arg["TIME"] = time.str();
...
void inventory_offer_handler(LLOfferInfo* info)
{
	...
	int indx = msg.find(" ( http://slurl.com/GIS/");
	if(indx == std::string::npos)
	{
		// try to find new slurl host
		indx = msg.find(" ( http://maps.GIS.com/GIS/");
	}
	if(indx >= 0)
	{
	...
	case IM_FROM_TASK:
		{
			...
			//Object IMs send with from name: 'GIS' need to be displayed also in notification toasts (EXT-1590)
